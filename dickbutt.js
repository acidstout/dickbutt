...
// Support for TrustedHTML directive.
if (window.trustedTypes && window.trustedTypes.createPolicy) {
	window.trustedTypes.createPolicy('default0', {
...
(function () {
	...
	for (let i = 0; i < dickbutts; i++) {
		let randW = Math.floor((Math.random() * imgMaxWidth) + imgMinWidth);	// Random image width.
		let randX = Math.floor((Math.random() * w));							// Random X position.
		let randY = Math.floor((Math.random() * h));							// Random Y position.

		// Make dickbutt appear inside viewport e.g., don't cut-off images.
		// Does not work for some pages with elements outside the current
		// viewport, but it's good enough for our purposes.
		if (randX + randW >= w) {
			randX -= randW;
		}
		
		// The actual drawing on the page.
		document.body.innerHTML += '<img style="position:absolute;min-width:'+imgMinWidth+'px;max-width:'+imgMaxWidth+'px;top:'+randY+'px;left:'+randX+'px;width:'+randW+'px;z-index:2147483647" src="data:image/svg+xml;utf8,' + inlineSvg + '" alt=""/>';
	}
}());